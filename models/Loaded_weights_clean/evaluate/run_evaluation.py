...
# --- IMPORT ARCHITEKTURY ---
# Upewnij się, że plik backbone_iresnet.py jest w tym samym folderze
try:
...
BASE_FOLDER_LOCAL = '../../../../webface_112x112'  # Folder z datasetem (train/val/test)
LOCAL_WEIGHTS_PATH = 'w600k_r50_from_onnx.pth'  # Twój plik z wagami
BASE_DIR_LOCAL = '../../../../webface_112x112'   # Folder z datasetem (train/val/test)
TEST_SUBDIR = 'test'                             # Podfolder do testów
...
# =============================================================================
# CZĘŚĆ 1: ZAAWANSOWANE ŁADOWANIE MODELU (SKOPIOWANE Z load_and_test.py)
# =============================================================================
...
def reset_layer_params(layer, name):
    ...
    # print(f"   ♻️ Resetowanie warstwy: {name}") # Ograniczam logi
...
def smart_load_hybrid_v4(model, state_dict_path):
    ...
    # Re-inicjalizacja brakujących
    unassigned_target = set(target_keys) - matched_target_keys
    if unassigned_target:
        print(f"⚠️ Brak wag dla {len(unassigned_target)} warstw - reinicjalizacja.")
...
def load_clean_model():
    ...
    if os.path.exists(LOCAL_WEIGHTS_PATH):
        ...
        if not success:
            print("⚠️ Nie udało się załadować wag, model losowy!")
    else:
        print(f"⚠️ Brak pliku wag {LOCAL_WEIGHTS_PATH}, model losowy!")
...
# =============================================================================
# CZĘŚĆ 2: LOGIKA EWALUACJI I PREPROCESSING
# =============================================================================
...
def get_embedding_raw(model, img_bgr):
    ...
    # 1. Preprocessing (Resize -> RGB -> Transpose -> Normalize)
    try:
        # Zakładamy, że obraz wchodzi już przycięty lub skalujemy do 112x112
        ...
        # 4. Normalizacja L2 (Ważne dla Cosine Similarity)
...
def discover_file_structure(local_test_path):
    ...
    for jpg_path in tqdm(all_jpg_files, desc="Indeksowanie"):
        ...
        # Sprawdzamy czy jest para JSON (opcjonalne, zależnie od datasetu, ale tu wymagane dla okluzji)
...
def build_faiss_gallery(model, identity_to_imgfolders, image_pairs):
    ...
    for id_path in tqdm(identity_paths, desc="Tworzenie galerii"):
        ...
        # Bierzemy PIERWSZĄ POŁOWĘ jako galerię
        ...
        if id_embeddings:
            # Uśredniamy embeddingi dla tożsamości (Prototyp)
...
def apply_occlusion(image, landmarks_dict, bbox):
    ...
    try:
        ...
        # Fallback jeśli landmarki są błędne - zwracamy oryginał
        return image.copy() 
...
def run_occlusion_evaluation(model, identity_to_imgfolders, image_pairs):
    ...
    with open(RESULTS_CSV, 'w', newline='', encoding='utf-8') as f:
        ...
        for id_path in tqdm(identity_paths, desc="Testowanie"):
            ...
            # Bierzemy DRUGĄ POŁOWĘ jako zapytania (Query)
            split_point = max(1, len(image_folder_paths) // 2)
            ...
            for img_folder_path in query_folders:
                ...
                D, I = index.search(query_vector, 1) # Top 1
    ...
    if total_queries > 0:
        ...
        print("\n⚠️ Ostrzeżenie: Nie przetworzono żadnych zapytań.")

# =============================================================================
# MAIN
# =============================================================================
def main():
    # 1. Załaduj model (ze szpitalem i naprawą nazw)
