...
MODEL_PATH = './phase2_best.pth'
METRICS_DIR = 'metrics_local_occlusion_odbicie'
...
def load_model():
    ...
    if len(missing) > 0:
        print(f"Brakujące klucze (sprawdź czy to nie core): {missing[:5]}...")
    
    cbam_in = any("cbam_layer" in k for k in new_state.keys())
    if cbam_in:
        print("Wykryto i załadowano wagi CBAM.")
    else:
        print("OSTRZEŻENIE: Brak wag CBAM w pliku .pth!")
