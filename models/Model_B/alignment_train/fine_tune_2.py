...
# Importujemy brakujące klasy z Twojego kodu (żeby to był jeden plik)
# Jeśli masz je w innym pliku, po prostu je zaimportuj
# Tutaj wklejam skrótowo definicje, które są niezbędne

# --- KONFIGURACJA ETAPU 2 ---
BASE_DIR = '../../../../webface_112x112'
TRAIN_DIR = os.path.join(BASE_DIR, 'train')
VAL_DIR = os.path.join(BASE_DIR, 'val')
DEBUG_DIR = 'training_photos_stage2'
# Ścieżka do Twojego modelu z Etapu 1 (upewnij się, że plik tam jest!)
PRETRAINED_PATH = 'best_model_merged.pth' 
...
# --- ZMIANY DLA "AGRESYWNEGO" TRENINGU ---
# Bardzo mały LR dla Backbone (żeby nie zniszczyć tego co już umie)
# Trochę większy dla Głowicy (bo ona startuje od zera/losowo)
LR_BACKBONE = 1e-5    
LR_HEAD = 1e-3        

# --- KLASY (Te same co wcześniej) ---
...
def main():
    ...
    # 1. Tworzymy PUSTY szkielet (architekturę)
    print(f"🏗️ Inicjalizacja architektury IResNet...")
    backbone = load_clean_model() 
    
    # 2. NADPISUJEMY wagi Twoim modelem
    print(f"🔄 Wczytywanie Twoich wag z: {PRETRAINED_PATH}...")
    if os.path.exists(PRETRAINED_PATH):
        # Wczytujemy słownik wag
        state_dict = torch.load(PRETRAINED_PATH, map_location=DEVICE)
        
        # Czasem zapisują się z prefiksem "module." lub "backbone.", czyścimy to:
        new_state_dict = {}
        for k, v in state_dict.items():
            name = k.replace("module.", "").replace("backbone.", "")
            new_state_dict[name] = v
            
        # Ładujemy do backbone
        ...
        return # Kończymy, bo nie ma sensu trenować od zera

    # 3. Odmrażamy WSZYSTKO (Backbone też się uczy, ale powoli)
    ...
    # Dataset
    # WAŻNE: Tu musi być definicja klasy OcclusionFaceDataset dostępna w kodzie
    # Zakładam, że ją masz wyżej wklejoną
    ...
    # Optimizer - Różne LR!
    optimizer = optim.SGD([
        {'params': model.backbone.parameters(), 'lr': LR_BACKBONE}, # Wolno (1e-5)
        {'params': model.arcface.parameters(), 'lr': LR_HEAD},      # Szybciej (1e-3)
        {'params': model.aux_head.parameters(), 'lr': LR_HEAD}
    ], momentum=0.9, weight_decay=5e-4)
    
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=2)
    # Zmieniamy nazwę pliku zapisu, żeby nie nadpisać starego 'best_model_merged.pth'
    early_stopping = EarlyStopping(patience=PATIENCE, path='best_model_stage2.pth') 
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (img, lbl, msk) in enumerate(loop):
            ...
            loss = criterion_cls(logits, lbl) + 0.1 * criterion_aux(m_pred, msk)
        ...
        model.eval()
        val_total_loss = 0.0
        # Opcjonalnie: Licznik Accuracy (jak w poprzednim kodzie)
        ...
        with torch.no_grad():
            for img, lbl, msk in val_loader:
                ...
                val_total_loss += (v_loss_cls + 0.1 * v_loss_aux).item()
...
if __name__ == "__main__":
    # Upewnij się, że masz zdefiniowane klasy OcclusionFaceDataset i EarlyStopping!
    main()
