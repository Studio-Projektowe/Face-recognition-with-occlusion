...
# Importujemy architekturę SE-ResNet (musi to być ten sam plik co przy treningu!)
...
def initialize_services():
    """Ładuje klienta GCS i model PyTorch (IResNet50 + SE). Zwraca (model, bucket)."""
    ...
    print("Ładowanie wytrenowanego modelu SE-IResNet (PyTorch)...")
    try:
        # ZMIANA 1: Ścieżka do Twojego nowego, wytrenowanego modelu
        state_path = 'best_model_se_occlusion.pth'
        
        if not os.path.exists(state_path):
            print(f"BŁĄD: Nie znaleziono pliku wag: {state_path}")
            print("Upewnij się, że trening się zakończył i plik jest w tym katalogu.")
            sys.exit(1)
        ...
        # Trening zapisywał: torch.save(model.backbone.state_dict(), ...)
        # Więc ładujemy to bezpośrednio.
        checkpoint = torch.load(state_path, map_location=DEVICE)
        
        # Obsługa ewentualnego zagnieżdżenia w kluczu 'state_dict' (dla bezpieczeństwa)
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            checkpoint = checkpoint['state_dict']
            
        # Ładowanie do modelu
        model.load_state_dict(checkpoint)
        print(f"✅ Pomyślnie załadowano wagi z: {state_path}")

        model.to(DEVICE)
        model.eval() # Bardzo ważne: wyłącza Dropout i ustawia BN w tryb inferencji
