...
# Import architektury z Twojego projektu
try:
...
# Folder z danymi testowymi (musi zawierać podfoldery ID i pliki .json z landmarkami)
# Dostosuj tę ścieżkę do swojego serwera!
BASE_TEST_FOLDER = '../../../../webface_112x112/test' 

# Pliki wyjściowe
METRICS_DIR = 'metrics_cbam'
FAISS_INDEX_FILE = os.path.join(METRICS_DIR, 'my_model.index')
FAISS_MAPPING_FILE = os.path.join(METRICS_DIR, 'my_model_mapping.json')
RESULTS_CSV = os.path.join(METRICS_DIR, 'evaluation_results_stage2_top3.csv') # Zmiana nazwy pliku
OCCLUSION_OUTPUT_DIR = 'occlusion_photos_eval'

# Parametry okluzji
OCCLUSION_SIZE = 20
K_NEIGHBORS = 3 # NOWOŚĆ: Liczba sąsiadów do sprawdzenia

# Transformacja (musi być identyczna jak w treningu!)
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])

# --- 1. INICJALIZACJA MODELU (CUSTOM) ---

def initialize_custom_model():
    """Ładuje Twój wytrenowany backbone."""
    print(f"🏗️ Budowanie architektury iResNet...")
    model = load_clean_model() # To ładuje pusty szkielet lub w600k
    
    print(f"🔄 Wczytywanie Twoich wag z: {MODEL_PATH}...")
    if os.path.exists(MODEL_PATH):
        try:
            state_dict = torch.load(MODEL_PATH, map_location=DEVICE)
            # Obsługa zapisu z DataParallel lub samego backbone
            new_state_dict = {}
...
def get_embedding(model, image_bgr):
    ...
    """
    try:
        # 1. Preprocessing (Resize do 112x112)
        ...
        # 3. Transformacja (ToTensor + Normalize)
        ...
        # 5. Normalizacja L2 (Kluczowe dla ArcFace!)
...
def discover_file_structure(local_test_path):
    ...
    for jpg_path in tqdm(all_jpg_files, desc="Indeksowanie"):
        ...
        # Sprawdzamy czy jest JSON (potrzebny do okluzji)
        ...
        # Zapisujemy parę (jeśli json nie istnieje, będzie None)
...
def build_faiss_gallery(model, identity_to_imgfolders, image_pairs):
    ...
    for id_path in tqdm(identity_paths, desc="Przetwarzanie ID"):
        ...
        # Bierzemy PIERWSZĄ połowę zdjęć jako galerię (Clean)
        ...
        if id_embeddings:
            # Uśredniamy embeddingi dla jednej osoby (centroid)
...
def apply_occlusion(image, landmarks_dict, bbox):
    ...
    try:
        ...
        # Awaryjnie po prostu rysujemy pasek na środku
        h, w = image.shape[:2]
...
def run_occlusion_evaluation(model, identity_to_imgfolders, image_pairs):
    ...
    identity_paths = list(identity_to_imgfolders.keys())
    total_queries = 0
    correct_top1 = 0 # Liczymy dodatkowo Top-1
    correct_top3 = 0 # Główna metryka teraz
    ...
    with open(RESULTS_CSV, 'w', newline='', encoding='utf-8') as f:
        ...
        header = ["ground_truth_id", "top1_id", "sim1", "top2_id", "sim2", "top3_id", "sim3", "found_in_top3"]
        ...
        for id_path in tqdm(identity_paths, desc="Test Okluzji"):
            ...
            # Bierzemy DRUGĄ połowę zdjęć (Query)
            split_point = max(1, len(image_folder_paths) // 2)
            ...
            for img_folder_path in query_folders:
                ...
                # Pobranie landmarków z JSON (jeśli istnieją)
                landmarks = None
                bbox = None
                ...
                # 3. Szukanie w FAISS (ZMODYFIKOWANE DLA K=3)
                query_vector = np.expand_dims(query_embedding, axis=0).astype('float32')
                
                # Szukamy K najbliższych sąsiadów
                ...
                # Iterujemy po zwróconych K wynikach
                for k in range(K_NEIGHBORS):
                    # FAISS zwraca -1 jeśli nie ma wystarczającej liczby wektorów w indeksie
                    if I[0][k] == -1:
                    ...
                    # Sprawdzamy, czy ten wynik to nasza poszukiwana osoba
                ...
                # Dodatkowo sprawdzamy Top-1 (pierwszy element listy)
                if top_identities[0] == ground_truth_id:
                    correct_top1 += 1
                
                total_queries += 1

                # Przygotowanie wiersza do CSV
...
def main():
    ...
    # 3. Budowanie Galerii (Czyste zdjęcia)
    if not build_faiss_gallery(model, identity_to_imgfolders, image_pairs):
        return
    
    # 4. Testowanie (Zdjęcia z okluzją) - teraz Top-3
