...
class IResNetWithCBAM(nn.Module):
    ...
    def __init__(self, block, layers, dropout=0, num_features=512, zero_init_residual=False,
        ...
        # ZMIANA: CBAM po Layer 3
        # Layer 3 w IResNet50 (przy Expansion=1) ma 256 kanałów
    ...
    def forward(self, x):
        ...
        # ZMIANA: Aplikujemy CBAM tutaj (po layer3, przed layer4)
        x = self.cbam_layer(x)
...
def load_model_with_cbam(pretrained_path):
    ...
    # Przetwarzanie kluczy (usuwanie prefixu 'backbone.')
    for k, v in pretrained_state.items():
        # Usuń 'backbone.' jeśli istnieje (gdy ładujemy z FaceModelWithAux)
        if k.startswith('backbone.'):
            name = k[9:] # usuń pierwsze 9 znaków 'backbone.'
        elif k.startswith('module.'): # obsługa DataParallel
            name = k[7:]
        else:
            name = k
            
        if name in model_state:
            # Sprawdź kształt (na wypadek gdybyś zmieniał architekturę)
            ...
            # To normalne dla warstw CBAM lub głowicy (head), których nie ma w backbone
            pass
...
def calculate_rank_k(model, val_loader, num_samples=500):
    ...
    with torch.no_grad():
        ...
        for img, lbl, _ in val_loader:
            ...
            img = img.to(DEVICE)
            # Use labels=None to get raw features (not arcface logits)
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        # Save the wrapping model or backbone? Let's save backbone to stay compatible
...
class FaceModelWithAux(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        # Aux head expects (B, 512) which matches features shape
...
class OcclusionFaceDataset(Dataset):
    def __init__(self, root_dirs, transform=None):
        ...
        for d in root_dirs:
            if os.path.exists(d):
                # Zakładamy strukturę: root / identity / image_folder / image.jpg
                files = glob.glob(os.path.join(d, "*", "*", "*.jpg"))
                self.image_paths.extend(files)
            else:
                print(f"WARNING: Folder {d} does not exist!")

        # Fix class parsing: take 3rd element from the end as ID
        # .../train/id_123/img_folder/face.jpg -> id_123
    ...
    def __getitem__(self, idx):
        ...
        # CRITICAL FIX: Ensure label is valid
        if label == -1:
        ...
        # CRITICAL FIX: Data Poisoning prevention
        if img is None:
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        # Validation Loss (Raw softmax for stability check)
        model.eval()
        val_total_loss = 0.0
        with torch.no_grad():
            for img, lbl, msk in val_loader:
                img, lbl, msk = img.to(DEVICE), lbl.to(DEVICE), msk.to(DEVICE)
                # Pass labels for ArcFace loss calculation during val to check convergence
                logits, m_pred = model(img, lbl) 
