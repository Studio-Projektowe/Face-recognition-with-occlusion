...
MODEL_REPO = "Eliza921/IResNet50_CBAM_in_Blocks"
MODEL_FILENAME = "Aligned_Pretrained_CBAM_Block_v2.pth"
...
def main():
    ...
    for case_name, data in TEST_CASES.items():
        ...
        t1 = process_image(data['path1'])
        t2 = process_image(data['path2'])
        ...
        print(f" Image 1: {os.path.basename(data['path1'])}")
        print(f" Image 2: {os.path.basename(data['path2'])}")
        print(f" Similarity: {score:.4f}")
        print(f" Verdict: {'[YES]' if is_same else '[NO]'} {verdict} (Threshold: {THRESHOLD})\n")
